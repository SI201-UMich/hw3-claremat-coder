# Name: Clare Mathison
# Student ID: 7537 9681
# Email: claremat@umich.edu
# Who or what you worked with on this homework (including generative AI like ChatGPT): Sofia Ayala
# If you worked with generative AI also add a statement for how you used it. I asked ChatGPT for instructions of how to correctly commit my code
# e.g.:
...
class CouponDispenser:
    ...
    def issue_coupon(self, name):
        ...
        if name in self.customer_roster:
            ...
            coupon_card = self.coupon_cards[coupon_index]
            return f"That name already has a coupon: {coupon_card}"
        else:
            coupon_index = random.randint(0, len(self.coupon_cards) - 1)
            self.customer_roster.append(name)
            self.issued_indices.append(coupon_index)
            coupon_card = self.coupon_cards[coupon_index]
            return f"The coupon for {name} is: {coupon_card}"
        ...
        while True: 
            ...
            if user_input == "exit":
                ...
                for i in range(len(self.customer_roster)):
                    ...
                    coupon_card = self.coupon_cards[coupon_index]
                    print(f"{self.customer_roster[i]}: {coupon_card}")
            else: 
